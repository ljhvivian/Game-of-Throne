...
#include <iostream>
using namespace std;
int Gameworld2::_randomProduceBomb=0;
int Gameworld2::_monsterCount=0;
int Gameworld2::_bombCount = 0;
Gameworld2::Gameworld2(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Gameworld2)
{
    ui->setupUi(this);
    _game.initWorld("D:\\Bomber3\\mapfile1.txt");//初始化world
    _gameRecord.initNoteTable();//初始化notetable
    //以下是对时钟的初始化
...
void Gameworld2::paintEvent(QPaintEvent *e){
    Q_UNUSED(e);
    QPainter *pa;
    pa = new QPainter();
    pa->begin(this);
    ...
    if(_game.getWP2()) {_game.clearbomb();_game.clearMonster();_game.clearObj();close();}
}

void Gameworld2::keyPressEvent(QKeyEvent *e)
{
